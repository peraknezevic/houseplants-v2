...
export const createPage = async (
  ...
  pageFields.forEach(
    (field) => (data[field.name] = formData.get(field.name) as string),
  );
...
export const updateGenera = async (
  previousState: z.infer<typeof generaSchema>,
  formData: FormData,
) => {
  let success = false;
...
export const createPlantProfile = async (
  ...
  plantProfileFields.forEach((field) => {
    if (field.type === "number") {
      data[field.name] = Number(formData.get(field.name)) as number;
    } else {
      data[field.name] = formData.get(field.name) as string;
    }
  });
...
export const createPlant = async (
  ...
  plantFields.forEach((field) => {
    ...
    if (field.type === "checkbox") {
      data[field.name] = Boolean(formData.get(field.name)) as boolean;
    } else {
      data[field.name] = formData.get(field.name) as string;
    }
  });
...
export const updatePlant = async (
  previousState: z.infer<typeof plantSchema>,
  formData: FormData,
) => {
  let success = false;
